    ...
    <div class="card-body">
        ...
        @if(is_null($pagare->mandato_id))
            ...
            @if($pagare->estado_id !== 3)
                ...
                <a href="{{ route('pagare.mandato.create', [$pagare->id]) }}" class="card-link">Crear</a>
            @endif
        @else
            <p class="card-text">{{ $pagare->mandato->full_nombre}}</p>
            <p class="card-text">RUT : {{ formatoRut($pagare->mandato->rut) }}</p>
            <a href="{{ route('pagare.mandato.edit', [$pagare->mandato_id]) }}" class="card-link float-right">Editar</a>
        @endif
    </div>
</div>